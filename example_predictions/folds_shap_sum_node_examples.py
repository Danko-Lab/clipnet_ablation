...
sys.path.append("/home2/ayh8/clipnet_scripts/clipnet/")
import utils
...
# Load sequences
ref_fp = os.path.join(
    "/home2/ayh8/data/gse110638/tfbs_sampling/",
...
explain_fasta_fp = os.path.join(
    "/home2/ayh8/data/gse110638/examples/", "examples_reference_seq.fna.gz"
)
...
model_fps = list(Path("/home2/ayh8/ensemble_models/").glob("*.h5"))
models = [tf.keras.models.load_model(fp, compile=False) for fp in model_fps]
explainers = [
    shap.DeepExplainer((model.input, model.output[1]), onehot_ref) for model in models
]
...
np.savez_compressed(
    out_dir.joinpath("folds_examples_sum_node.npz"),
    **scaled_explanations,
)

np.savez_compressed(
    out_dir.joinpath("folds_examples_seqs.npz"),
...
np.savez_compressed(
    out_dir.joinpath("folds_qtl_examples_sum_node.npz"),
    **scaled_qtl_explanations,
)
