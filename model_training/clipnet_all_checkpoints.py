...
import tqdm
from clipnet import cgen, utils
import clipnet_arch as nn
...
class EveryNEpochModelCheckpoint(tf.keras.callbacks.ModelCheckpoint):
    def __init__(self, *args, checkpoint_frequency=1, **kwargs):
...
class CLIPNET:
    ...
    def __set_model_locations(self, resume_checkpoint):
        ...
        if resume_checkpoint is not None:
            self.model_filepath = os.path.join(
                self.model_dir, f"{self.name}_resume_epoch_{{epoch:03d}}.hdf5"
            )
            self.best_model_filepath = os.path.join(
                self.model_dir, f"{self.name}_resume_best.hdf5"
            )
        else:
            self.model_filepath = os.path.join(
                self.model_dir, f"{self.name}_epoch_{{epoch:03d}}.hdf5"
            )
            self.best_model_filepath = os.path.join(
                self.model_dir, f"{self.name}_best.hdf5"
            )
    ...
    def fit(
        ...
        with self.strategy.scope():
            ...
            best_checkp = tf.keras.callbacks.ModelCheckpoint(
            ...
            periodic_checkp = EveryNEpochModelCheckpoint(
                self.model_filepath,
                verbose=0,
